...
public class Vector {
    ...
    public static Vector sumUp(Vector...args) {
        if (args.length <= 1) {
            throw new IllegalArgumentException("debe haber más de un argumento para esta función");
        }
        Vector r = args[0];
        IntStream.range(1, r.getDimension())
                .forEach(i -> r.sumar(args[i]));
        return r;
    }
